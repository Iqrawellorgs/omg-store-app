...
class RestaurantController extends GetxController implements GetxService {
  ...
  void getAttributeList(Product? product) async {
    ...
    if (product != null && product.images != null) {
      _savedImages.addAll(product.images ?? []);
    }
  ...
  Future<void> updateRestaurant(Restaurant restaurant, List<String> cuisines, String token,
    ...
    Response response = await restaurantRepo.updateRestaurant(
        restaurant, cuisines, _pickedLogo, _pickedCover, token, translation);
    if (response.statusCode == 200) {
      Get.back();
  ...
  Future<void> addProduct(Product product, bool isAdd) async {
    ...
    Response response = await restaurantRepo.addProduct(
        product, _pickedLogo, _rawImages, _savedImages, isAdd, tags);
    if (response.statusCode == 200) {
  ...
  Future<Product?> getProductDetails(int productId) async {
    ...
    update();
    Response response = await restaurantRepo.getProductDetails(productId);
    if (response.statusCode == 200) {
