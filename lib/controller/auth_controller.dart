...
class AuthController extends GetxController implements GetxService {
  ...
  bool _inZone = false;
  int _zoneID = 0;
  int _businessIndex = Get.find<SplashController>().configModel!.businessPlan != null &&
  ...
  String _storeMinTime = '00';
  String _storeMaxTime = '00';
  ...
  void resetBusiness() {
    _businessIndex = Get.find<SplashController>().configModel!.businessPlan != null &&
            Get.find<SplashController>().configModel!.businessPlan!.commission == 0
        ? 1
        : 0;
  ...
  Future<ResponseModel> setUpBusinessPlan(BusinessPlanBody businessPlanBody) async {
    ...
    if (response.statusCode == 200) {
      ...
      Future.delayed(
          const Duration(seconds: 2), () => Get.offAllNamed(RouteHelper.getSignInRoute()));
    } else {
      ...
      if (Get.find<SplashController>().configModel!.businessPlan!.subscription == 1) {
        ...
        _profileModel = ProfileModel(
          ...
          subscriptionOtherData:
              SubscriptionOtherData.fromJson(response.body['subscription_other_data']),
        );
        Get.toNamed(RouteHelper.getSubscriptionViewRoute());
      } else {
        responseModel =
            ResponseModel(false, 'subscription_not_available_please_contact_with_admin'.tr);
      }
    } else {
      responseModel = ResponseModel(false, response.statusText);
    }
  ...
  Future<ResponseModel> resetPassword(
      String? resetToken, String? email, String password, String confirmPassword) async {
    _isLoading = true;
    update();
  ...
  Future<void> registerRestaurant(RestaurantBody restaurantBody) async {
    ...
    update();
    Response response =
        await authRepo.registerRestaurant(restaurantBody, _pickedLogo, _pickedCover);
    if (response.statusCode == 200) {
  ...
  Future<void> zoomToFit(GoogleMapController controller, List<LatLng> list,
    ...
    controller.moveCamera(CameraUpdate.newCameraPosition(
        CameraPosition(target: centerBounds, zoom: GetPlatform.isWeb ? 10 : 16)));
    ...
    if (text.isNotEmpty) {
      ...
      if (response.statusCode == 200 && response.body['status'] == 'OK') {
        ...
        response.body['predictions']
            .forEach((prediction) => _predictionList.add(PredictionModel.fromJson(prediction)));
      } else {
        showCustomSnackBar(response.body['error_message'] ?? response.bodyString);
      }
    }
